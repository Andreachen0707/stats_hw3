...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores,cache_2 = fc_relu_forward(scores,W2,b2)
        scores,cache_3 = fc_forward(scores,W3,b3)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dx_1, grads['W3'],grads['b3'] = fc_backward(dout,cache_3)
        dx_2, grads['W2'],grads['b2'] = fc_relu_backward(dx_1,cache_2)
        dx_3, grads['W1'],grads['b1'] = conv_relu_pool_backward(dx_2,cache_1)

        loss = loss + 0.5*self.reg*np.sum(W1*W1)+ 0.5*self.reg*np.sum(W2*W2)+0.5*self.reg*np.sum(W3*W3)
